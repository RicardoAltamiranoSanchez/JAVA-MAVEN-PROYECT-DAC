...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	...
	{view:'combo',id:'IdEmpleados',name:'IdEmpleados',value:'',placeholder:'<%=idiomaModulo.getProperty("IdEmpleados") %>',label:'<%=idiomaModulo.getProperty("IdEmpleados") %>',labelPosition:'top',required:true,options:[],yCount:'3'},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'IdFirmas',header:'<%=idiomaModulo.getProperty("IdFirmas")%>',width:250,sort:'string'},
	{id:'IdEmpleados',header:'<%=idiomaModulo.getProperty("IdEmpleados")%>',width:400,sort:'string'}]}]};
var vacio={id:'Vacio'};
...
/* $$('IdFirmas').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('IdFirmas').setValue(nuevo); });
$$('IdEmpleados').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('IdEmpleados').setValue(nuevo); }); */
inicial();
...
var IdEmpleados = $$("IdEmpleados").getPopup().getList();IdEmpleados.clearAll();IdEmpleados.load("servlet/EmpleadosServlet?Accion=getEmpleados&filter[value]=");
