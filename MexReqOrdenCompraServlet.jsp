...
<%@ page import="Objetos.MexReqOrdenCompra"%>
...
private void imprimeJson(HttpServletResponse response, MexReqOrdenCompra objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexReqOrdenCompra> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexReqOrdenCompra objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexReqOrdenCompraServlet.jsp");
	log.append(" Evento:");
...
try {
	...
	if(request.getParameter("Accion").equals("Buscar") || request.getParameter("Accion").equals("Autorizar") || request.getParameter("Accion").equals("Rechazar")) {
		if(request.getParameter("Accion").equals("Autorizar")) {
			gson = new Gson();
			try {
				MexReqOrdenCompra[] ids = gson.fromJson(request.getParameter("Ids"), MexReqOrdenCompra[].class);
		...
		resultados = sentencia.executeQuery("select A.* from MexReqOrdenCompra as A" + whereInicio + where.toString());
		ArrayList<MexReqOrdenCompra> info = new ArrayList<MexReqOrdenCompra>();
