...
<%@ page import="Objetos.MexReqEntradasAdmon"%>
...
private void imprimeJson(HttpServletResponse response, MexReqEntradasAdmon objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexReqEntradasAdmon> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexReqEntradasAdmon objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexReqEntradasAdmonServlet.jsp");
	log.append(" Evento:");
...
MexReqEntradasAdmon objeto = null;
int ultimoId = 0;
try {
	...
	if(request.getParameter("Accion").equals("Buscar") || request.getParameter("Accion").equals("Autorizar") || request.getParameter("Accion").equals("Rechazar")) {
		if(request.getParameter("Accion").equals("Autorizar")) {
			gson = new Gson();
			try {
				MexReqEntradasAdmon[] ids = gson.fromJson(request.getParameter("Ids"), MexReqEntradasAdmon[].class);
		...
		objeto = new MexReqEntradasAdmon();
		...
		resultados = sentencia.executeQuery("select A.* from MexReqEntradasAdmon as A" + whereInicio + where.toString());
		ArrayList<MexReqEntradasAdmon> info = new ArrayList<MexReqEntradasAdmon>();
...
/* MexReqEntradasAdmonServlet.jsp */
%>
