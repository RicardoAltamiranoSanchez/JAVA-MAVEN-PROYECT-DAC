...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'Nombre',name:'Nombre',value:'',placeholder:'<%=idiomaModulo.getProperty("Nombre")%>',label:'<%=idiomaModulo.getProperty("Nombre")%>',labelPosition:'top',attributes:{maxlength:25},required:true},
	// COMBO en lugar de text se cambia a combo para mostrar una lista de otra tabla
	{view:'combo',id:'IdClasificacionActivos',name:'IdClasificacionActivos',value:'',placeholder:'<%=idiomaModulo.getProperty("IdClasificacionActivos")%>',label:'<%=idiomaModulo.getProperty("IdClasificacionActivos")%>',labelPosition:'top',required:true,options:[],yCount:'5'},
	{view:'combo',id:'Plantilla',name:'Plantilla',value:'',placeholder:'<%=idiomaModulo.getProperty("Plantilla") %>',label:'<%=idiomaModulo.getProperty("Plantilla") %>',labelPosition:'top',required:true,options:["GENERALES","PCs y LAPTOPS","SOFTWARE","TRANSPORTES"]},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	...
	{id:'IdClasificacionActivos',header:'<%=idiomaModulo.getProperty("IdClasificacionActivos")%>',sort:'string'},
	{id:'Plantilla',header:'<%=idiomaModulo.getProperty("Plantilla")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'TIPOS DE ACTIVO'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
function enviar(mensaje,accion){var valores=$$('Forma').getValues();webix.ajax().post('servlet/TiposActivoServlet',valores,{error:function(text,xml,XmlHttpRequest){webix.message(errorComunicacion);$$('Mensaje').setValue('<font color="red"><strong>'+errorComunicacion+'</strong></font>');},success:function(text,data,XmlHttpRequest){if(accion=='Consultar'){$$('Forma').parse(data.json());webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');}else{if(data.json().error){webix.message(errorEjecutarAccion);$$('Mensaje').setValue('<font color="red"><strong>'+errorEjecutarAccion+'</strong></font>');webix.message(data.json().log);$$('Confirmacion').clearAll();$$('Confirmacion').add({campo:'ERROR',value: data.json().log});}else{webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');if(accion=='Guardar'||accion=='Modificar'){$$('Confirmacion').clearAll();setConfirmacion(data.json());if(accion=='Guardar'){$$('Confirmacion').add({campo:'ESTATUS',value:guardado});guardarDespues();}else{$$('Confirmacion').add({campo:'ESTATUS',value:modificado});modificarDespues();}}else if(accion=='Borrar'){borrarDespues();}else if(accion=='Buscar'){$$('PanelTabla').show();$$('Tabla').clearAll();$$('Tabla').parse(data.json());var altura=$$('PanelTabla').$height-$$('TablaTools').$height;$$('Tabla').define('height',altura);$$('Tabla').resize();}}}}});}
function setConfirmacion(info){
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Nombre")%>',value: info.Nombre});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("IdClasificacionActivos")%>',value: info.IdClasificacionActivos});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Plantilla")%>',value: info.Plantilla});
}
$$('Nombre').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Nombre').setValue(nuevo); });
$$('IdClasificacionActivos').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('IdClasificacionActivos').setValue(nuevo); });
$$('Plantilla').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Plantilla').setValue(nuevo); });
// COMBO llenar lista de clasificaciones
var listaIdClasificacionActivos = $$("IdClasificacionActivos").getPopup().getList();listaIdClasificacionActivos.clearAll();listaIdClasificacionActivos.load("servlet/ClasificacionActivosServlet?Accion=getClasificacionActivos&filter[value]=");
inicial();
</script>
