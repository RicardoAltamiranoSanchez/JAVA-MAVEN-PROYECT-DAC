...
<%@ page import="Objetos.MexProveedores"%>
...
private void imprimeJson(HttpServletResponse response, MexProveedores objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexProveedores> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexProveedores objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexProveedoresServlet.jsp");
	log.append(" Evento:");
...
try {
	...
	if(request.getParameter("Accion").equals("Buscar")) {
		...
		String whereInicio = " where " + seguridad.getNivel(0,session,new String[]{""}) + " order by Alias";
		boolean entro = false;
		boolean agregaOr = false;
		
		objeto = new MexProveedores();
		...
		resultados = sentencia.executeQuery("select A.* from MexProveedores as A" + whereInicio + where.toString());
		ArrayList<MexProveedores> info = new ArrayList<MexProveedores>();
		...
		gson = new Gson();
		try {
			MexProveedores[] ids = gson.fromJson(request.getParameter("Ids"), MexProveedores[].class);
			for(int i = 0; i < ids.length; i++) {
				sentencia.executeUpdate("insert into MexProveedoresApoyo (Quien,Registro,Borrar,IdOrigen,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios) select '" + session.getAttribute("IdUsuario") + "',now(),'Si',Id,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios from MexProveedores where Id = '" + ids[i].getId() + "'");
				sentencia.executeUpdate("delete from MexProveedores where Id = '" + ids[i].getId() + "'");
			}
		} catch(JsonSyntaxException e1) {
			MexProveedores ids = gson.fromJson(request.getParameter("Ids"), MexProveedores.class);
			sentencia.executeUpdate("insert into MexProveedoresApoyo (Quien,Registro,Borrar,IdOrigen,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios) select '" + session.getAttribute("IdUsuario") + "',now(),'Si',Id,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios from MexProveedores where Id = '" + ids.getId() + "'");
			sentencia.executeUpdate("delete from MexProveedores where Id = '" + ids.getId() + "'");
		}
		objeto = new MexProveedores();
		imprimeJson(response,objeto);
	}
	else if(request.getParameter("Accion").equals("Modificar")) {
		sentencia.executeUpdate("update MexProveedores set Fecha='" + valoresDefault.getFecha(request.getParameter("Fecha")) + "',Alias='" + request.getParameter("Alias") + "',Nombre='" + request.getParameter("Nombre") + "',Rfc='" + request.getParameter("Rfc") + "',Direccion='" + request.getParameter("Direccion") + "',NumeroExterior='" + request.getParameter("NumeroExterior") + "',NumeroInterior='" + request.getParameter("NumeroInterior") + "',Colonia='" + request.getParameter("Colonia") + "',CodigoPostal='" + request.getParameter("CodigoPostal") + "',Telefono='" + request.getParameter("Telefono") + "',Ciudad='" + request.getParameter("Ciudad") + "',Estado='" + request.getParameter("Estado") + "',Pais='" + request.getParameter("Pais") + "',GiroEmpresa='" + request.getParameter("GiroEmpresa") + "',SitioInternet='" + request.getParameter("SitioInternet") + "',Contacto='" + request.getParameter("Contacto") + "',Extension='" + request.getParameter("Extension") + "',Celular='" + request.getParameter("Celular") + "',Email='" + request.getParameter("Email") + "',DaCredito='" + request.getParameter("DaCredito") + "',DiasCredito='" + valoresDefault.getEntero(request.getParameter("DiasCredito")) + "',Banco1='" + request.getParameter("Banco1") + "',CuentaBancaria1='" + request.getParameter("CuentaBancaria1") + "',Clabe1='" + request.getParameter("Clabe1") + "',Moneda1='" + request.getParameter("Moneda1") + "',Banco2='" + request.getParameter("Banco2") + "',CuentaBancaria2='" + request.getParameter("CuentaBancaria2") + "',Clabe2='" + request.getParameter("Clabe2") + "',Moneda2='" + request.getParameter("Moneda2") + "',Estatus='" + request.getParameter("Estatus") + "',Comentarios='" + request.getParameter("Comentarios") + "' where Id = '" + request.getParameter("id") + "'");
		sentencia.executeUpdate("insert into MexProveedoresApoyo (Quien,Registro,IdOrigen,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios) select '" + session.getAttribute("IdUsuario") + "',now(),Id,U,G,E,Bloquear,Fecha,Alias,Nombre,Rfc,Direccion,NumeroExterior,NumeroInterior,Colonia,CodigoPostal,Telefono,Ciudad,Estado,Pais,GiroEmpresa,SitioInternet,Contacto,Extension,Celular,Email,DaCredito,DiasCredito,Banco1,CuentaBancaria1,Clabe1,Moneda1,Banco2,CuentaBancaria2,Clabe2,Moneda2,Estatus,Comentarios from MexProveedores where Id = '" + request.getParameter("id") + "'");
		objeto = new MexProveedores();
		imprimeJson(response,objeto);
	}
	else if(request.getParameter("Accion").equals("getProveedores")) {
		resultados = sentencia.executeQuery("select Id, concat(alias,' ',Nombre) as Proveedor from MexProveedores where alias like '" + request.getParameter("filter[value]") + "%' order by Alias and Estatus = 'Activo'");
		ArrayList<MexProveedores> info = new ArrayList<MexProveedores>();
		while(resultados.next()) {
			objeto = new MexProveedores();
			objeto.setId(resultados.getString("Id"));
