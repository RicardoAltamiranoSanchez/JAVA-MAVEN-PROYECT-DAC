...
var forma={view:'form',id:'Forma',width:250,height:'100%',elements:[
	...
	{view:'combo',id:'IdUsuario',name:'IdUsuario',value:'',placeholder:'<%=idiomaModulo.getProperty("IdUsuario")%>',label:'<%=idiomaModulo.getProperty("IdUsuario")%>',labelPosition:'top',options:[],yCount:'3',required:true,hidden:false},
{view:'label',id:'Mensaje',label:'',align:'center'},{view:'toolbar', cols:[{},{view:'button',id:'NuevoBoton',value:etiquetaNuevo,width:85, click:'nuevo'},{view:'button',value:etiquetaBuscar,width:85,click:'buscar'}]}]};
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',height:15,cols:[
	...
	{id:'UsuarioSolicitante',header:'<%=idiomaModulo.getProperty("UsuarioSolicitante")%>',width:100,sort:'string'},
	{id:'NombreSolicitante',header:'NOMBRE SOLICITANTE',width:230,sort:'string'},
...
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'SOLICITUDES DE VACACIONES/DIAS SIN GOCE PENDIENTES'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
function guardar(){$$('Confirmacion').clearAll();$$('Forma').setValues({Accion:'ConsultarSolicitudesPendientesEmpleadosGdl'},true); if($$('Forma').validate()) { enviar(registroGuardado,'Guardar'); $$('Confirmacion').show();}}
...
function buscar(){$$('Forma').setValues({Accion:'ConsultarSolicitudesPendientesEmpleadosGdl'},true);enviar(busquedaFinalizada,'Buscar');}
