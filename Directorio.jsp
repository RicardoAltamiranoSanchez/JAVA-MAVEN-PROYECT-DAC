...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	...
	{view:'combo',id:'Division',name:'Division',value:'',placeholder:'<%=idiomaModulo.getProperty("Division") %>',label:'<%=idiomaModulo.getProperty("Division")%>',required:true,yCount:'5',options:["DLV","DAC","MWS","JMF","NET","MCS","MAM","SEG","OLG"]},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	...
	{id:'Nextel',header:'<%=idiomaModulo.getProperty("Nextel")%>',sort:'string'},
	{id:'IdNextel',header:'<%=idiomaModulo.getProperty("IdNextel")%>',sort:'string'},
	{id:'Correo',header:'<%=idiomaModulo.getProperty("Correo")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
...
function BuscarInfo(){
	$$('Forma').setValues({Accion:'BuscarInfo'},true); var valores=$$('Forma').getValues(); webix.ajax().get('servlet/DirectorioServlet',valores,{error:function(text,xml,XmlHttpRequest){webix.message(errorComunicacion);$$('Mensaje').setValue('<font color="red"><strong>'+errorComunicacion+'</strong></font>');},success:function(text,data,XmlHttpRequest){ if(data.json().Puesto != ""){ $$('Forma').parse(data.json());webix.message("BUSQUEDA EXITOSA");$$('Mensaje').setValue('<font color="green"><strong>BUSQUEDA EXITOSA</strong></font>'); $$('Puesto').disable(); $$('Division').disable(); $$('Estacion').disable(); }}});
}
