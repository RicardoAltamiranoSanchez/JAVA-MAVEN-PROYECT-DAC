...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'TipoDocumento',name:'TipoDocumento',value:'',placeholder:'<%=idiomaModulo.getProperty("TipoDocumento")%>',label:'<%=idiomaModulo.getProperty("TipoDocumento")%>',required:true},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'TipoDocumento',header:'<%=idiomaModulo.getProperty("TipoDocumento")%>',fillspace:true,sort:'string'},
	{id:'Nombre',header:'<%=idiomaModulo.getProperty("Nombre")%>',sort:'string'},
	{id:'IdEmpleadosGrupo',header:'<%=idiomaModulo.getProperty("IdEmpleadosGrupo")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'DOCUMEMPLEADOSGRUPO'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
$$('TipoDocumento').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('TipoDocumento').setValue(nuevo); });
