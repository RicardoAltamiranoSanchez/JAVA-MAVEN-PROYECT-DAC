...
<script>
var servlet='MexReqFinalizadosServlet.jsp';
...
function autorizar() {
	webix.confirm({title:'ENTRADA',ok:si,cancel:no,type:'confirm-error',text:'Realizar entrada a las requisiciones seleccionadas?',callback:function(result){if(result){var ids=$$('Tabla').getSelectedId();if(ids == null){webix.message(seleccioneUnRegistro);$$('Mensaje').setValue('<font color="blue"><strong>'+seleccioneUnRegistro+'</strong></font>');}else{$$('Forma').setValues({Accion:'Autorizar',Ids:ids},true);enviar('Autorizar','Autorizar');}}else{$$('Tabla').clearSelection();}}});
}
function rechazar() {
	webix.confirm({title:'Rechazar',ok:si,cancel:no,type:'confirm-error',text:'Rechazar las requisiciones seleccionadas?',callback:function(result){if(result){var ids=$$('Tabla').getSelectedId();if(ids == null){webix.message(seleccioneUnRegistro);$$('Mensaje').setValue('<font color="blue"><strong>'+seleccioneUnRegistro+'</strong></font>');}else{$$('Forma').setValues({Accion:'Rechazar',Ids:ids},true);enviar('Rechazar','Rechazar');}}else{$$('Tabla').clearSelection();}}});
}
