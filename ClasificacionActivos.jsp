...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'Nombre',name:'Nombre',value:'',placeholder:'<%=idiomaModulo.getProperty("Nombre")%>',label:'<%=idiomaModulo.getProperty("Nombre")%>',labelPosition:'top',attributes:{maxlength:99},required:true},
	// VALIDA NÚMEROS la variable alertaNumeros incluye el texto que se adjunta a la etiqueta
	{view:'text',id:'Clave',name:'Clave',value:'',placeholder:'<%=idiomaModulo.getProperty("Clave")%>',label:'<%=idiomaModulo.getProperty("Clave")%>'+alertaNumeros,labelPosition:'top',attributes:{maxlength:2},validate:"isNumber",required:true},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	...
	{id:'Clave',header:'<%=idiomaModulo.getProperty("Clave")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'CLASIFICACIÓN DE ACTIVOS'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
