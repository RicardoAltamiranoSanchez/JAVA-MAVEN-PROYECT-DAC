...
var forma={view:'form',id:'Forma',width:1024,scroll:'y',height:'100%',elements:[
	{view:'datepicker',id:'Fecha',name:'Fecha',value: new Date(),placeholder:'<%=idiomaModulo.getProperty("Fecha")%>',label:'<%=idiomaModulo.getProperty("Fecha")%>',stringResult:true,format:webix.Date.dateToStr("%Y-%m-%d"),required:true},
	{view:'text',id:'Titulo',name:'Titulo',value:'',placeholder:'<%=idiomaModulo.getProperty("Titulo")%>',label:'<%=idiomaModulo.getProperty("Titulo")%>',required:true},
	{view:'textarea',id:'Resumen',name:'Resumen',value:'',placeholder:'<%=idiomaModulo.getProperty("Resumen")%>',label:'<%=idiomaModulo.getProperty("Resumen")%>',required:true},
	{view:'textarea',id:'Noticia',name:'Noticia',value:'',placeholder:'<%=idiomaModulo.getProperty("Noticia")%>',label:'<%=idiomaModulo.getProperty("Noticia")%>',required:true},
	...
	{view:'uploader',id:'Archivo',name:'Archivo',value:'',link:'mylist',placeholder:'<%=idiomaModulo.getProperty("Archivo")%>',label:'<%=idiomaModulo.getProperty("Archivo")%>',required:true,upload:'servlet/SubirNoticiasServlet',autosend:true,multiple:false},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	...
	{id:'Titulo',header:'<%=idiomaModulo.getProperty("Titulo")%>',sort:'string'},
	{id:'Archivo',header:'<%=idiomaModulo.getProperty("Archivo")%>',sort:'string'},
	{id:'Resumen',header:'<%=idiomaModulo.getProperty("Resumen")%>',sort:'string'},
	{id:'Noticia',header:'<%=idiomaModulo.getProperty("Noticia")%>',sort:'string'},
	{id:'Ocultar',header:'<%=idiomaModulo.getProperty("Ocultar")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'NOTICIAS'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
$$('Resumen').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Resumen').setValue(nuevo); });
$$('Noticia').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Noticia').setValue(nuevo); });
</script>
