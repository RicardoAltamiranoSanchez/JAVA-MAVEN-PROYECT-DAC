...
var forma={view:'form',id:'Forma',width:400,scroll:'y',height:'100%',elements:[
	{view:'combo',id:'CategoriaFirma',name:'CategoriaFirma',value:'',placeholder:'<%=idiomaModulo.getProperty("CategoriaFirma") %>',label:'<%=idiomaModulo.getProperty("CategoriaFirma") %>',labelPosition:'top',options:[],required:true,yCount:'3'},
	
	{view:'uploader',id:'Archivo',name:'Archivo',value:'',link:'mylist',placeholder:'<%=idiomaModulo.getProperty("Archivo")%>',label:'<%=idiomaModulo.getProperty("Archivo")%>',upload:'servlet/SubirBannerCategoriaFirmaServlet',autosend:true,required:true,multiple:false},
		
{view:'label',id:'Mensaje',label:'',align:'center'},{view:'toolbar', cols:[{view:'button',value:etiquetaNuevo,id:'BotonNuevo',width:85, click:'nuevo'},{view:'button',id:'BotonGuardar',value:etiquetaGuardar,width:85,click:'guardar'},{view:'button',id:'BotonModificar',value:etiquetaModificar,width:85,click:'modificar',hidden:true},{view:'button',id:'BotonBuscar',value:etiquetaBuscar,width:85,click:'buscar',hidden:true}]}]};
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'CategoriaFirma',header:'<%=idiomaModulo.getProperty("CategoriaFirma")%>',sort:'string',hidden:true},
...
var listaCategoriaFirma = $$("CategoriaFirma").getPopup().getList();listaCategoriaFirma.clearAll();listaCategoriaFirma.load("servlet/FirmasGruposServlet?Accion=getFirmasGruposDeAreaBanner&filter[value]=");

//$$('Archivo').attachEvent('onAfterFileAdd',function(){ $$('Archivo').setValue($$('CategoriaFirma').getValue()); });

$$('BotonNuevo').hide();
$$('BotonBuscar').hide();
$$('Archivo').disable();
$$('CategoriaFirma').attachEvent('onChange',function(){
	$$('Archivo').define('formData',{Llave:$$('CategoriaFirma').getValue()});
	if ($$('CategoriaFirma').getValue() != ''){
				$$('Archivo').enable();
		}
});
$$('Archivo').attachEvent('onItemClick',function(){ 
		if ($$('CategoriaFirma').getValue() == ''){
