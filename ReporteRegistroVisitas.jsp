...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	...
	{view:'combo',id:'IdRegistroEntradasAlmacenGdl',name:'IdRegistroEntradasAlmacenGdl',value:'',placeholder:'Visitante',label:'Visitante',labelPosition:'top',required:false,options:[],yCount:'3'},
...
function enviar(mensaje,accion){var valores=$$('Forma').getValues();webix.ajax().post('ReporteRegistroVisitasServlet.jsp',valores,{error:function(text,xml,XmlHttpRequest){webix.message(errorComunicacion);$$('Mensaje').setValue('<font color="red"><strong>'+errorComunicacion+'</strong></font>');},success:function(text,data,XmlHttpRequest){if(accion=='Consultar'){$$('Forma').parse(data.json());webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');}else{if(data.json().error){webix.message(errorEjecutarAccion);$$('Mensaje').setValue('<font color="red"><strong>'+errorEjecutarAccion+'</strong></font>');webix.message(data.json().log);$$('Confirmacion').clearAll();$$('Confirmacion').add({campo:'ERROR',value: data.json().log});}else{webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');if(accion=='Guardar'||accion=='Modificar'){$$('Confirmacion').clearAll();setConfirmacion(data.json());if(accion=='Guardar'){$$('Confirmacion').add({campo:'ESTATUS',value:guardado});guardarDespues();}else{$$('Confirmacion').add({campo:'ESTATUS',value:modificado});modificarDespues();}}else if(accion=='Borrar'){borrarDespues();}else if(accion=='Buscar'){$$('PanelTabla').show();$$('Tabla').clearAll();$$('Tabla').parse(data.json());var altura=$$('PanelTabla').$height-$$('TablaTools').$height;$$('Tabla').define('height',altura);$$('Tabla').resize();}}}}});}
function setConfirmacion(info){
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Fecha")%>',value: info.Fecha});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("IdRegistroEntradasAlmacenGdl")%>',value: info.IdRegistroEntradasAlmacenGdl});
}
$$('IdRegistroEntradasAlmacenGdl').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('IdRegistroEntradasAlmacenGdl').setValue(nuevo); });

var listaIdRegistroEntradasAlmacenGdl = $$("IdRegistroEntradasAlmacenGdl").getPopup().getList();listaIdRegistroEntradasAlmacenGdl.clearAll();listaIdRegistroEntradasAlmacenGdl.load("RegistroEntradasAlmacenGdlServlet.jsp?Accion=getRegistroEntradasAlmacenGdlInactivos&filter[value]=");
...
function excel(){
	...
	if (FechaInicio == null){
		...
		document.location = 'ReporteRegistroVisitasExcel.jsp?Accion=Guardar' +
				'&AuxFecha=' + $$('AuxFecha').getValue() + 
				'&Fecha=' + $$('Fecha').getValue() + 
				'&FechaA=' + $$('FechaA').getValue() +
				'&IdRegistroEntradasAlmacenGdl=' + $$('IdRegistroEntradasAlmacenGdl').getValue();
