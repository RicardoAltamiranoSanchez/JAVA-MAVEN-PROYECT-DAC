...
var forma={view:'form',id:'Forma',width:400,scroll:'y',height:'100%',elements:[
	{view:'combo',id:'Sitio',name:'Sitio',value:'',placeholder:'<%=idiomaModulo.getProperty("Sitio") %>',label:'<%=idiomaModulo.getProperty("Sitio") %>',labelPosition:'top',options:[],required:true,yCount:'3'},
	{view:'combo',id:'Posicion',name:'Posicion',value:'',placeholder:'<%=idiomaModulo.getProperty("Posicion") %>',label:'<%=idiomaModulo.getProperty("Posicion") %>',labelPosition:'top',options:[],required:true,yCount:'3'},
	{view:'uploader',id:'Archivo',name:'Archivo',value:'',link:'mylist',placeholder:'<%=idiomaModulo.getProperty("Archivo")%>',label:'<%=idiomaModulo.getProperty("Archivo")%>',upload:'servlet/SubirBannerServlet',autosend:true,required:true,multiple:false},
	{borderless: true, view:"list", id:"mylist", type:"uploader", autoheight:true, minHeight: 50},
	{cols:[
		{view:'combo',id:'Ocultar',name:'Ocultar',value:'',placeholder:'<%=idiomaModulo.getProperty("Ocultar") %>',label:'<%=idiomaModulo.getProperty("Ocultar") %>',labelPosition:'top',options:["No","Si"],required:true,yCount:'3'},
		{view:'text',id:'Link',name:'Link',value:'',placeholder:'<%=idiomaModulo.getProperty("Link")%>',label:'<%=idiomaModulo.getProperty("Link")%>',labelPosition:'top'},
	]},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'Sitio',header:'<%=idiomaModulo.getProperty("Sitio")%>',sort:'string',hidden:true},
	{id:'Posicion',header:'<%=idiomaModulo.getProperty("Posicion")%>',width:75,sort:'string'},
	{id:'Archivo',header:'<%=idiomaModulo.getProperty("Archivo")%>',fillspace:true,sort:'string',
		template:"#Archivo# [<a href='http://banners1.mcs-holding.com/#Archivo#' target='Banner1'>Mirror 1</a>] [<a href='http://banners2.mcs-holding.com/?Banner=#id#' target='Banner2'>Mirror 2</a>]"
	},
	{id:'Ocultar',header:'<%=idiomaModulo.getProperty("Ocultar")%>',width:50,sort:'string'},
	{id:'Link',header:'<%=idiomaModulo.getProperty("Link")%>',width:250,sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'BANNER'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
function inicial(){for(var i=1;i<variablesFijas.length;i++){$$(variablesFijas[i].campo).setValue(variablesFijas[i].valor);}}
function nuevo(){$$('mylist').clearAll();$$('BotonGuardar').show();$$('BotonModificar').hide();$$('Forma').clear();$$('Confirmacion').clearAll();$$('Tabla').clearAll();$$('Mensaje').setValue(nada);$$('Vacio').show();iniciales();}
...
function setConfirmacion(info){
	...
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Link")%>',value: info.Link});
	$$('mylist').clearAll();
}
inicial();

//cargar los datos de sitio y posicion
var listaPosicion = $$("Posicion").getPopup().getList();listaPosicion.clearAll();listaPosicion.load("servlet/PosicionServlet?Accion=getPosicion&filter[value]=");
var listaSitio = $$("Sitio").getPopup().getList();listaSitio.clearAll();listaSitio.load("servlet/SitioServlet?Accion=getSitio&filter[value]=");
</script>
