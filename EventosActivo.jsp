...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	...
	{view:'text',id:'FechEvento',name:'FechEvento',value:'',placeholder:'<%=idiomaModulo.getProperty("FechEvento")%>',label:'<%=idiomaModulo.getProperty("FechEvento")%>',required:true},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'Descripcion',header:'<%=idiomaModulo.getProperty("Descripcion")%>',fillspace:true,sort:'string'},
	{id:'FechEvento',header:'<%=idiomaModulo.getProperty("FechEvento")%>',sort:'string'},
	{id:'IdInventario',header:'<%=idiomaModulo.getProperty("IdInventario")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'EVENTOSACTIVO'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
$$('FechEvento').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('FechEvento').setValue(nuevo); });
