...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'GrupoEmpleados',name:'GrupoEmpleados',value:'',placeholder:'<%=idiomaModulo.getProperty("GrupoEmpleados")%>',label:'<%=idiomaModulo.getProperty("GrupoEmpleados")%>',labelPosition:'top',attributes:{maxlength:255},required:true},
	// COMBO en lugar de text se cambia a combo para mostrar una lista de otra tabla
	{view:'combo',/* inputWidth:300, */id:'IdEmpleados',name:'IdEmpleados',value:'',placeholder:'<%=idiomaModulo.getProperty("IdEmpleados") %>',label:'<%=idiomaModulo.getProperty("IdEmpleados") %>',labelPosition:'top',required:true,options:[],yCount:'5'},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},/* OCULTANDO BOTON MODIFICAR *//* {view:'button',value:etiquetaModificar,width:85,click:'consultar'} */]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'GrupoEmpleados',header:'<%=idiomaModulo.getProperty("GrupoEmpleados")%>',fillspace:false,width:'300',sort:'string'},
	{id:'IdEmpleados',header:'<%=idiomaModulo.getProperty("IdEmpleados")%>',fillspace:true,sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'AGRUPADOR DE EMPLEADOS'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
function guardarDespues(){$$('IdEmpleados').setValue('');inicial(); /* VOLVIENDO A LLENAR LA LISTA PARA QUE NO ESCOJA A ALGUIEN QUE YA REGISTRO  */var listaIdEmpleados = $$("IdEmpleados").getPopup().getList();listaIdEmpleados.clearAll();listaIdEmpleados.load("servlet/EmpleadosServlet?Accion=getEmpleadosPatronEstacion&filter[value]=");}
...
function borrarDespues(){$$('Forma').setValues({Accion:'Buscar'},true);enviar(registroEliminado,'Buscar'); /* VOLVIENDO A LLENAR LA LISTA PARA QUE INGRESE AL EMPLEADO QUE FUE ELIMINADO DE LA TABLA  */var listaIdEmpleados = $$("IdEmpleados").getPopup().getList();listaIdEmpleados.clearAll();listaIdEmpleados.load("servlet/EmpleadosServlet?Accion=getEmpleadosPatronEstacion&filter[value]=");}
...
//$$('IdAreasGrupo').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('IdAreasGrupo').setValue(nuevo); });
// COMBO llenar lista de empleados
var listaIdEmpleados = $$("IdEmpleados").getPopup().getList();listaIdEmpleados.clearAll();listaIdEmpleados.load("servlet/EmpleadosServlet?Accion=getEmpleadosPatronEstacion&filter[value]=");
inicial();
</script>
