...
<script>
//var servlet='servlet/RegistroEntradasAlmacenMxlServlet';
...
//DEFINIENDO FORMAS
	...
	var formaacc={view:'form',id:'FormaAcc',width:'100%',height:'100%',elements:[
		...
		{cols:[
			{rows:[
				...
				//Aquí se define de forma constante la estacion, en este caso MXL
				{view:'text',id:'EstacionEnt',name:'Estacion',value:'MXL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
			]},
	...
	var formasal1={view:'form',id:'FormaSal1',width:'100%',height:'100%',elements:[
			{rows:[
				...
				/* {view:'combo',id:'IdRegistroEntradasAlmacenMxl',name:'IdRegistroEntradasAlmacenMxl',value:'',placeholder:IdRegistroEntradasAlmacenMxl,label:IdRegistroEntradasAlmacenMxl,labelWidth:100,labelPosition:'left',options:[],yCount:'3',required:true}, */
					{view:"label", 
	...
	var formachec={view:'form',id:'FormaChec',width:'100%',height:'100%',elements:[
		//Estacion
		{view:"label", 
		    label: "<span style='font-size:80px; color:red;font-family:verdana;font-style:bold'>MXL</span>", 
		    height:125,
		    align:"center"},
	  ...
	  //Botones
		...
		{view:'text',id:'EstacionChec',name:'Estacion',value:'MXL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
	]};
...
var formasal2={view:'form',id:'FormaSal2',width:800,height:800,elements:[
	...
	{view:'text',id:'EstacionSal',name:'Estacion',value:'MXL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
...
//FUNCIONES DE FORMAS
function inicial(){for(var i=1;i<variablesFijas.length;i++){$$(variablesFijas[i].campo).setValue(variablesFijas[i].valor);}/*DEFINIENDO SIEMPRE ESTACION AQUI*/$$('EstacionEnt').setValue('MXL');$$('EstacionSal').setValue('MXL');if(modoSubforma){$$('FrameSubforma').hide();}}
...
//Checador
//function guardarentradachec(){$$('FormaChec').setValues({Accion:'GuardarEntrada'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntrada');webix.alert('Se registró el Acceso del Número de Empleado: '+$$('NumEmpleado').getValue());$$('FormaChec').clear();$$('EstacionChec').setValue('MXL');}}
...
function guardarentradachec(){$$('FormaChec').setValues({Accion:'GuardarEntrada'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntrada');$$('FormaChec').clear();$$('EstacionChec').setValue('MXL');}}
...
function guardarsalidachec(){$$('FormaChec').setValues({Accion:'GuardarSalida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarSalida');/*webix.alert("<span style='font-size:30px; color:red;font-family:verdana;font-style:bold'>Salida Registrada</span>");*/$$('FormaChec').clear();$$('EstacionChec').setValue('MXL');}}

function guardarentcomchec(){$$('FormaChec').setValues({Accion:'GuardarEntradaComida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntradaComida');$$('FormaChec').clear();$$('EstacionChec').setValue('MXL');}}
function guardarsalcomchec(){$$('FormaChec').setValues({Accion:'GuardarSalidaComida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarSalidaComida');$$('FormaChec').clear();$$('EstacionChec').setValue('MXL');}}
...
//BLOQUEADO, FUNCIONA COMBO
/* $$('IdRegistroEntradasAlmacenMxl').attachEvent("onItemClick", function(id, e){
    //code
	var servletIdRegistroEntradasAlmacenMxl='RegistroEntradasAlmacenMxlServlet.jsp';var listaIdRegistroEntradasAlmacenMxl=$$('IdRegistroEntradasAlmacenMxl').getPopup().getList();listaIdRegistroEntradasAlmacenMxl.clearAll();listaIdRegistroEntradasAlmacenMxl.load(servletIdRegistroEntradasAlmacenMxl + '?Accion=getIdRegistroEntradasAlmacenMxlActivos&filter[value]=');
	//$$('VentanaFormaSal2').show();
}); */

    //code
function cargaSalidas(){$$('BotonesSalidas').clearAll();$$('BotonesSalidas').load('RegistroEntradaVisitasAlmacenesServlet.jsp?Accion=getIdRegistroEntradasAlmacenesActivosEst&filter[value]=,MXL');}
//$$('VentanaFormaSal2').show();
cargaSalidas();
activaReloj();
...
//RegistroEntradasAlmacenMxl.jsp
%>