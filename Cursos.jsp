...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'Clave',name:'Clave',value:'',labelPosition:'top',placeholder:'<%=idiomaModulo.getProperty("Clave")%>',label:'<%=idiomaModulo.getProperty("Clave")%>',required:true},
	{view:'text',id:'Curso',name:'Curso',value:'',labelPosition:'top',placeholder:'<%=idiomaModulo.getProperty("Curso")%>',label:'<%=idiomaModulo.getProperty("Curso")%>',required:true},
	{view:'text',id:'HorasCurso',name:'HorasCurso',labelPosition:'top',value:'',placeholder:'<%=idiomaModulo.getProperty("HorasCurso")%>',label:'<%=idiomaModulo.getProperty("HorasCurso")%>',required:true},
	{view:'text',id:'HorarioCurso',name:'HorarioCurso',labelPosition:'top',value:'',placeholder:'<%=idiomaModulo.getProperty("HorarioCurso")%>',label:'<%=idiomaModulo.getProperty("HorarioCurso")%>',required:true},
	{view:'combo',id:'TipoCurso',name:'TipoCurso',value:'',labelPosition:'top',placeholder:'<%=idiomaModulo.getProperty("TipoCurso") %>',label:'<%=idiomaModulo.getProperty("TipoCurso")%>',required:true,yCount:'5',options:["DGR","MCS","VOLARIS"]},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'Clave',header:'<%=idiomaModulo.getProperty("Clave")%>',fillspace:true,sort:'string'},
	{id:'Curso',header:'<%=idiomaModulo.getProperty("Curso")%>',sort:'string'},
	{id:'HorasCurso',header:'<%=idiomaModulo.getProperty("HorasCurso")%>',sort:'string'},
	{id:'HorarioCurso',header:'<%=idiomaModulo.getProperty("HorarioCurso")%>',sort:'string'},
	{id:'TipoCurso',header:'<%=idiomaModulo.getProperty("TipoCurso")%>',sort:'string'}]}]};
	
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'CURSOS'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
function enviar(mensaje,accion){var valores=$$('Forma').getValues();webix.ajax().post('servlet/CursosServlet',valores,{error:function(text,xml,XmlHttpRequest){webix.message(errorComunicacion);$$('Mensaje').setValue('<font color="red"><strong>'+errorComunicacion+'</strong></font>');},success:function(text,data,XmlHttpRequest){if(accion=='Consultar'){$$('Forma').parse(data.json());webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');}else{if(data.json().error){webix.message(errorEjecutarAccion);$$('Mensaje').setValue('<font color="red"><strong>'+errorEjecutarAccion+'</strong></font>');webix.message(data.json().log);$$('Confirmacion').clearAll();$$('Confirmacion').add({campo:'ERROR',value: data.json().log});}else{webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');if(accion=='Guardar'||accion=='Modificar'){$$('Confirmacion').clearAll();setConfirmacion(data.json());if(accion=='Guardar'){$$('Confirmacion').add({campo:'ESTATUS',value:guardado});guardarDespues();}else{$$('Confirmacion').add({campo:'ESTATUS',value:modificado});modificarDespues();}}else if(accion=='Borrar'){borrarDespues();}else if(accion=='Buscar'){$$('PanelTabla').show();$$('Tabla').clearAll();$$('Tabla').parse(data.json());var altura=$$('PanelTabla').$height-$$('TablaTools').$height;$$('Tabla').define('height',altura);$$('Tabla').resize();}}}}});}
function setConfirmacion(info){
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Clave")%>',value: info.Clave});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Curso")%>',value: info.Curso});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("HorasCurso")%>',value: info.HorasCurso});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("HorarioCurso")%>',value: info.HorarioCurso});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("TipoCurso")%>',value: info.TipoCurso});
}
$$('Clave').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Clave').setValue(nuevo); });
$$('Curso').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Curso').setValue(nuevo); });
$$('HorasCurso').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('HorasCurso').setValue(nuevo); });
$$('HorarioCurso').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('HorarioCurso').setValue(nuevo); });
$$('TipoCurso').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('TipoCurso').setValue(nuevo); });
inicial();
</script>
