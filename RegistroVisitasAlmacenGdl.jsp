...
//DEFINIENDO FORMAS (RESOLUCIÓN 1366 x 768)
	...
	var formaacc={view:'form',id:'FormaAcc',width:'100%',height:'100%',elements:[
		...
		{cols:[
			{rows:[
				...
				//Aquí se define de forma constante la estacion, en este caso GDL
				{view:'text',id:'EstacionEnt',name:'Estacion',value:'GDL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
			]},
	...
	var formachec={view:'form',id:'FormaChec',width:'100%',height:'100%',elements:[
		//Estacion
		{view:"label", 
		    label: "<span style='font-size:60px; color:red;font-family:verdana;font-style:bold'>GDL</span>", 
		    height:100,
		    align:"center"},
    	...
    	{view:"label",
		    label:'<font size ="5"><font color="black"><strong>Ingresa tu usuario y contraseña de intranet</font color="black"></strong></font size ="5">',
		    height:30,
		    align:"center"},
		{cols:[
			{},
		...
		{view:'text',id:'EstacionChec',name:'Estacion',value:'GDL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
	]};
...
var formasal2={view:'form',id:'FormaSal2',width:800,height:800,elements:[
	...
	{view:'text',id:'EstacionSal',name:'Estacion',value:'GDL',placeholder:'Estacion',label:'Estacion',labelWidth:130,width:1100,height:100,labelPosition:'top',required:false,hidden:true},
...
//FUNCIONES DE FORMAS
function inicial(){for(var i=1;i<variablesFijas.length;i++){$$(variablesFijas[i].campo).setValue(variablesFijas[i].valor);}/*DEFINIENDO SIEMPRE ESTACION AQUI*/$$('EstacionEnt').setValue('GDL');$$('EstacionSal').setValue('GDL');if(modoSubforma){$$('FrameSubforma').hide();}}
...
//Checador
//function guardarentradachec(){$$('FormaChec').setValues({Accion:'GuardarEntrada'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntrada');webix.alert('Se registró el Acceso del Número de Empleado: '+$$('NumEmpleado').getValue());$$('FormaChec').clear();$$('EstacionChec').setValue('GDL');}}
...
function guardarentradachec(){$$('FormaChec').setValues({Accion:'GuardarEntrada'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntrada');$$('FormaChec').clear();$$('EstacionChec').setValue('GDL');}}
...
function guardarsalidachec(){$$('FormaChec').setValues({Accion:'GuardarSalida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarSalida');/*webix.alert("<span style='font-size:30px; color:red;font-family:verdana;font-style:bold'>Salida Registrada</span>");*/$$('FormaChec').clear();$$('EstacionChec').setValue('GDL');}}

function guardarentcomchec(){$$('FormaChec').setValues({Accion:'GuardarEntradaComida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarEntradaComida');$$('FormaChec').clear();$$('EstacionChec').setValue('GDL');}}
function guardarsalcomchec(){$$('FormaChec').setValues({Accion:'GuardarSalidaComida'},true);if($$('FormaChec').validate()){enviarchec(datosGuardados,'GuardarSalidaComida');$$('FormaChec').clear();$$('EstacionChec').setValue('GDL');}}
...
/* $$('IdRegistroEntradasAlmacenGdl').attachEvent("onItemClick", function(id, e){
    ...
    //code
function cargaSalidas(){$$('BotonesSalidas').clearAll();$$('BotonesSalidas').load('RegistroEntradaVisitasAlmacenesServlet.jsp?Accion=getIdRegistroEntradasAlmacenesActivosEst&filter[value]=,GDL');}
//$$('VentanaFormaSal2').show();
cargaSalidas();
activaReloj();
