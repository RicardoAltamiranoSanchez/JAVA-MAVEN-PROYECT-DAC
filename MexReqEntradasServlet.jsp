...
<%@ page import="Objetos.MexReqEntradas"%>
...
private void imprimeJson(HttpServletResponse response, MexReqEntradas objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexReqEntradas> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexReqEntradas objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexReqEntradasServlet.jsp");
	log.append(" Evento:");
...
try {
	...
	if(request.getParameter("Accion").equals("Buscar") || request.getParameter("Accion").equals("Autorizar") || request.getParameter("Accion").equals("Rechazar")) {
		if(request.getParameter("Accion").equals("Autorizar")) {
			gson = new Gson();
			try {
				MexReqEntradas[] ids = gson.fromJson(request.getParameter("Ids"), MexReqEntradas[].class);
		...
		resultados = sentencia.executeQuery("select A.* from MexReqEntradas as A" + whereInicio + where.toString());
		ArrayList<MexReqEntradas> info = new ArrayList<MexReqEntradas>();
