...
<%@ page import="Objetos.MexRegistroFila"%>
...
private void imprimeJson(HttpServletResponse response, MexRegistroFila objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexRegistroFila> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexRegistroFila objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexRegistroFilaServlet.jsp");
	log.append(" Evento:");
...
try {
	...
	if(request.getParameter("Accion").equals("Buscar")) {
		...
		resultados = sentencia.executeQuery("select A.* from MexRegistroFila as A" + whereInicio + where.toString());
		ArrayList<MexRegistroFila> info = new ArrayList<MexRegistroFila>();
		...
		sentencia.executeUpdate("insert into MexRegistroFila (U,G,E,Bloquear,Gafete,FechaHora) values ('1','1','1','false','" + request.getParameter("Gafete") + "',now())",Statement.RETURN_GENERATED_KEYS);
		ResultSet indice = sentencia.getGeneratedKeys();
		while(indice.next()) {
			ultimoId = indice.getInt(1);
		}
		try { if(null!=indice)indice.close();} catch (SQLException e) {;}
		sentencia.executeUpdate("insert into MexRegistroFilaApoyo (Quien,Registro,IdOrigen,U,G,E,Bloquear,Gafete,FechaHora) select '1',now(),Id,U,G,E,Bloquear,Gafete,FechaHora from MexRegistroFila where Id = '" + ultimoId + "'");
