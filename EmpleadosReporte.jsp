...
var forma={view:'form',id:'Forma',width:500,scroll:'y',height:'100%',elements:[
	{view:'text',id:'NombreCompleto',name:'NombreCompleto',value:'',placeholder:'<%=idiomaModulo.getProperty("NombreCompleto")%>',label:'<%=idiomaModulo.getProperty("NombreCompleto")%>',labelPosition:'top'},
	{cols:[
		...
		{view:'combo',id:'IdJefeDirecto',name:'IdJefeDirecto',value:'<%=session.getAttribute("IdUsuario") %>',placeholder:'<%=idiomaModulo.getProperty("IdJefeDirecto") %>',label:'<%=idiomaModulo.getProperty("IdJefeDirecto") %>',labelPosition:'top',options:[],yCount:'3',hidden:true},
	]},
	
	{view:'text',id:'Puesto',name:'Puesto',value:'',placeholder:'<%=idiomaModulo.getProperty("Puesto")%>',label:'<%=idiomaModulo.getProperty("Puesto")%>',hidden:true},
	{view:'combo',id:'Division',name:'Division',value:'',placeholder:'<%=idiomaModulo.getProperty("Division") %>',label:'<%=idiomaModulo.getProperty("Division") %>',required:true,options:[],yCount:'3',hidden:true},
	{view:'text',id:'Estacion',name:'Estacion',value:'',placeholder:'<%=idiomaModulo.getProperty("Estacion")%>',label:'<%=idiomaModulo.getProperty("Estacion")%>',hidden:true},
	{cols:[
	...
	{cols:[
		{view:'datepicker',id:'FechaIngreso',name:'FechaIngreso',value: '',placeholder:'<%=idiomaModulo.getProperty("FechaIngreso")%>',label:'<%=idiomaModulo.getProperty("FechaIngreso")%>',stringResult:true,format:webix.Date.dateToStr("%Y-%m-%d"),labelPosition:'top',hidden:true},
		{view:'combo',id:'Estatus',name:'Estatus',value:'ACTIVO',placeholder:'<%=idiomaModulo.getProperty("Estatus") %>',label:'<%=idiomaModulo.getProperty("Estatus")%>',labelPosition:'top',required:true,yCount:'5',options:[{id:'ACTIVO',value:'<%=idiomaModulo.getProperty("ACTIVO")%>'},{id:'INACTIVO',value:'<%=idiomaModulo.getProperty("INACTIVO")%>'}],hidden:true},
	]},
...
{view:'label',id:'Mensaje',label:'',align:'center'},{view:'toolbar', cols:[{view:'button',value:etiquetaNuevo,width:85, click:'nuevo',hidden:true},{view:'button',id:'BotonGuardar',value:etiquetaGuardar,width:85,click:'guardar',hidden:true},{view:'button',id:'BotonModificar',value:etiquetaModificar,width:85,click:'modificar',hidden:true},{view:'button',value:etiquetaBuscar,width:85,click:'buscar'}]}]};
var lista={view:'list',id:'Confirmacion',template:'#campo# : #value#'};
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaExcel,width:120,click:'excel'},{view:'button',value:etiquetaBorrar,width:85,click:'borrar',hidden:true},{view:'button',value:etiquetaModificar,width:85,click:'consultar',hidden:true}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
