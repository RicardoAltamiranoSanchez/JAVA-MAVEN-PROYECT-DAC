...
var forma={view:'form',id:'Forma',width:400,height:'100%',elements:[
	{view:'text',id:'Pais',name:'Pais',value:'',placeholder:'<%=idiomaModulo.getProperty("Pais")%>',label:'<%=idiomaModulo.getProperty("Pais")%>',labelPosition:'top',attributes:{maxlength:45},required:true},
	{view:'text',id:'CodigoPais',name:'CodigoPais',value:'',placeholder:'<%=idiomaModulo.getProperty("CodigoPais")%>',label:'<%=idiomaModulo.getProperty("CodigoPais")%>',labelPosition:'top',attributes:{maxlength:2},required:true},
	{view:'text',id:'Estado',name:'Estado',value:'',placeholder:'<%=idiomaModulo.getProperty("Estado")%>',label:'<%=idiomaModulo.getProperty("Estado")%>',labelPosition:'top',attributes:{maxlength:45},required:true},
	{view:'text',id:'Ciudad',name:'Ciudad',value:'',placeholder:'<%=idiomaModulo.getProperty("Ciudad")%>',label:'<%=idiomaModulo.getProperty("Ciudad")%>',labelPosition:'top',attributes:{maxlength:45},required:true},
	{view:'text',id:'CodigoCiudad',name:'CodigoCiudad',value:'',placeholder:'<%=idiomaModulo.getProperty("CodigoCiudad")%>',label:'<%=idiomaModulo.getProperty("CodigoCiudad")%>',labelPosition:'top',attributes:{maxlength:3},required:true},
	{view:'text',id:'Sucursal',name:'Sucursal',value:'',placeholder:'<%=idiomaModulo.getProperty("Sucursal")%>',label:'<%=idiomaModulo.getProperty("Sucursal")%>',labelPosition:'top',attributes:{maxlength:99},required:true},
	{view:'text',id:'DomicilioSuc',name:'DomicilioSuc',value:'',placeholder:'<%=idiomaModulo.getProperty("DomicilioSuc")%>',label:'<%=idiomaModulo.getProperty("DomicilioSuc")%>',labelPosition:'top',attributes:{maxlength:255},required:true},
...
var tabla={id:'PanelTabla',height:'100%',rows:[{view:'toolbar',id:'TablaTools',cols:[{view:'button',value:etiquetaBorrar,width:85,click:'borrar'},{view:'button',value:etiquetaModificar,width:85,click:'consultar'}]},{view:'datatable',id:'Tabla',width:'100%',select:'row',multiselect:true,columns:[
	{id:'Pais',header:'<%=idiomaModulo.getProperty("Pais")%>',fillspace:true,sort:'string'},
	{id:'CodigoPais',header:'<%=idiomaModulo.getProperty("CodigoPais")%>',sort:'string'},
	{id:'Estado',header:'<%=idiomaModulo.getProperty("Estado")%>',sort:'string'},
	{id:'Ciudad',header:'<%=idiomaModulo.getProperty("Ciudad")%>',sort:'string'},
	{id:'CodigoCiudad',header:'<%=idiomaModulo.getProperty("CodigoCiudad")%>',sort:'string'},
	{id:'Sucursal',header:'<%=idiomaModulo.getProperty("Sucursal")%>',sort:'string'},
	{id:'DomicilioSuc',header:'<%=idiomaModulo.getProperty("DomicilioSuc")%>',sort:'string'}]}]};
var vacio={id:'Vacio'};
webix.ui({id:'principal',type:'space',rows:[{type:'header', template:'UBICACIONES DEL GRUPO'},{view:'accordion',type:'wide',multi:true,height:'100%',cols:[{header:' ',body:forma},{animate:{type:'flip'},cells:[vacio,lista,tabla]}]}]});	
...
function enviar(mensaje,accion){var valores=$$('Forma').getValues();webix.ajax().post('servlet/UbicacionesGrupoServlet',valores,{error:function(text,xml,XmlHttpRequest){webix.message(errorComunicacion);$$('Mensaje').setValue('<font color="red"><strong>'+errorComunicacion+'</strong></font>');},success:function(text,data,XmlHttpRequest){if(accion=='Consultar'){$$('Forma').parse(data.json());webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');}else{if(data.json().error){webix.message(errorEjecutarAccion);$$('Mensaje').setValue('<font color="red"><strong>'+errorEjecutarAccion+'</strong></font>');webix.message(data.json().log);$$('Confirmacion').clearAll();$$('Confirmacion').add({campo:'ERROR',value: data.json().log});}else{webix.message(mensaje);$$('Mensaje').setValue('<font color="green"><strong>'+mensaje+'</strong></font>');if(accion=='Guardar'||accion=='Modificar'){$$('Confirmacion').clearAll();setConfirmacion(data.json());if(accion=='Guardar'){$$('Confirmacion').add({campo:'ESTATUS',value:guardado});guardarDespues();}else{$$('Confirmacion').add({campo:'ESTATUS',value:modificado});modificarDespues();}}else if(accion=='Borrar'){borrarDespues();}else if(accion=='Buscar'){$$('PanelTabla').show();$$('Tabla').clearAll();$$('Tabla').parse(data.json());var altura=$$('PanelTabla').$height-$$('TablaTools').$height;$$('Tabla').define('height',altura);$$('Tabla').resize();}}}}});}
function setConfirmacion(info){
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Pais")%>',value: info.Pais});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("CodigoPais")%>',value: info.CodigoPais});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Estado")%>',value: info.Estado});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Ciudad")%>',value: info.Ciudad});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("CodigoCiudad")%>',value: info.CodigoCiudad});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("Sucursal")%>',value: info.Sucursal});
	$$('Confirmacion').add({campo:'<%=idiomaModulo.getProperty("DomicilioSuc")%>',value: info.DomicilioSuc});
}
$$('Pais').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Pais').setValue(nuevo); });
$$('CodigoPais').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('CodigoPais').setValue(nuevo); });
...
$$('CodigoCiudad').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('CodigoCiudad').setValue(nuevo); });
$$('Sucursal').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('Sucursal').setValue(nuevo); });
$$('DomicilioSuc').attachEvent('onChange',function(nuevo, anterior){ nuevo = nuevo.toUpperCase(); $$('DomicilioSuc').setValue(nuevo); });
inicial();
</script>
<%=EncabezadoPie.getPie()%>
<% //UbicacionesGrupo.jsp %>
