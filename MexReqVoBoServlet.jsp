...
<%@ page import="Objetos.MexReqVoBo"%>
...
private void imprimeJson(HttpServletResponse response, MexReqVoBo objeto) throws IOException {
...
private void imprimeJson(HttpServletResponse response, ArrayList<MexReqVoBo> objeto) throws IOException {
	response.setContentType("application/json");
	PrintWriter out = response.getWriter();
	gson = new Gson();
	out.println(gson.toJson(objeto));
...
private void armaLog(MexReqVoBo objeto, Exception e) {
	objeto.setError(true);
	fechas = new Fechas();
	StringBuffer log = new StringBuffer();
	log.append("Serie:");
	log.append("MexReqVoBoServlet.jsp");
	log.append(" Evento:");
...
try {
	...
	if(request.getParameter("Accion").equals("Buscar") || request.getParameter("Accion").equals("Autorizar") || request.getParameter("Accion").equals("Rechazar")) {
		if(request.getParameter("Accion").equals("Autorizar")) {
			gson = new Gson();
			try {
				MexReqVoBo[] ids = gson.fromJson(request.getParameter("Ids"), MexReqVoBo[].class);
				for(int i = 0; i < ids.length; i++) {
					...
					sentencia.executeUpdate("update MexReqArchivos set Bloquear = 'true' where IdRequerimientos = '" + ids[i].getId() + "'");
					sentencia.executeUpdate("insert into MexReqArchivosApoyo (Quien,Registro,Borrar,IdOrigen,U,G,E,Bloquear,IdRequerimientos,Tipo,Archivo,Nombre) select '" + session.getAttribute("IdUsuario") + "',now(),'Si',Id,U,G,E,Bloquear,IdRequerimientos,Tipo,Archivo,Nombre from MexReqArchivos where IdRequerimientos = '" + ids[i].getId() + "'");
				...
				sentencia.executeUpdate("update MexReqArchivos set Bloquear = 'true' where IdRequerimientos = '" + ids.getId() + "'");
				sentencia.executeUpdate("insert into MexReqArchivosApoyo (Quien,Registro,Borrar,IdOrigen,U,G,E,Bloquear,IdRequerimientos,Tipo,Archivo,Nombre) select '" + session.getAttribute("IdUsuario") + "',now(),'Si',Id,U,G,E,Bloquear,IdRequerimientos,Tipo,Archivo,Nombre from MexReqArchivos where IdRequerimientos = '" + ids.getId() + "'");
		...
		String whereInicio = " where " + seguridad.getNivel(10,session,new String[]{""}) + " order by Id desc";
		boolean entro = false;
		boolean agregaOr = false;
		
		objeto = new MexReqVoBo();
		...
		if(entro) { whereInicio = " where " + seguridad.getNivel(10,session,new String[]{""}) + " and ("; where.append(")");}

		resultados = sentencia.executeQuery("select A.* from MexReqVoBo as A" + whereInicio + where.toString());
		ArrayList<MexReqVoBo> info = new ArrayList<MexReqVoBo>();
