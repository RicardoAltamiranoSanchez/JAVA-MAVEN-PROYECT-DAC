...
<script>
var servlet='MexReqOrdenCompraServlet.jsp';
...
{view:'toolbar',cols:[{view:'button',id:'BotonExcel2',value:'EXCEL',width:85, click:'excel'},{},{view:'button',id:'BotonPdf',value:'RECHAZAR',width:85, click:'rechazar'},{view:'button',id:'BotonExcel',value:'ORDEN DE COMPRA',width:85, click:'autorizar'}]},
...
function autorizar() {
	webix.confirm({title:'ORDEN DE COMPRA',ok:si,cancel:no,type:'confirm-error',text:'Generar orden de compra de las requisiciones seleccionadas?',callback:function(result){if(result){var ids=$$('Tabla').getSelectedId();if(ids == null){webix.message(seleccioneUnRegistro);$$('Mensaje').setValue('<font color="blue"><strong>'+seleccioneUnRegistro+'</strong></font>');}else{$$('Forma').setValues({Accion:'Autorizar',Ids:ids},true);enviar('Autorizar','Autorizar');}
	webix.ui({view:"window",id:'VentanaBuscadorMexReqVoBo',width:1000,height:450,position:'center',modal:true,head:{cols:[{},{view:'button', label:'X', width:50, click:("$$('VentanaBuscadorMexReqVoBo').close();")}]},body:{view:'iframe',id:'Ventana',src:'MexOrdenCompraPdf.jsp?Id=' + ids}}).show();
